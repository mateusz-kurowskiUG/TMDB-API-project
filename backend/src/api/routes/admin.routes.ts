...
import { movieIdParamValidator } from "../middleware/validators/id-param-validator";
...
adminRouter.patch(
  ...
  async (c) => {
    const { movieId: id } = c.req.valid("param");
    const {
      title,
      release_date,
      poster_path,
...
adminRouter.delete("/movies/:movieId", movieIdParamValidator(), async (c) => {
  const { movieId: id } = c.req.valid("param");
